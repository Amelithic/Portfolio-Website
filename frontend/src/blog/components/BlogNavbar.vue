...
const links: NavLink[] = [
  { label: 'Home', to: '/blog', exact: true },
  { label: 'All Posts', to: '/blog/posts' },
]

function submitSearch(): void {
  const q = search.value.trim()
  void router.push({ path: '/blog/posts', query: q ? { q } : {} })
}
</script>
...
<template>
  <Navigation :brand="siteConfig.brandName" :links="links">
    <template #leading>
      <span class="subapp-label">{{ siteCopy.nav.blog }}</span>
    </template>
    ...
    <template #after>
      <form class="nav-search" role="search" @submit.prevent="submitSearch">
        ...
        <input
          v-model="search"
          type="search"
          :placeholder="siteCopy.search.blog"
          :aria-label="siteCopy.search.blogAria"
        />
      </form>
